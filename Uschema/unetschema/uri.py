import re
from unetschema.error import URIParseError
...
class URI(object):
    ...
    def from_uri_string(cls, uri_string):
        """
        Parses unet uri into its components

        :param uri_string: format - unet://name:n$rank#id/path
                           optional modifiers:
...
def get_schema_regex():
    ...
    protocol = _named("protocol", re.escape(PROTOCOL))   # 'unet://'
...
def parse_unet_uri(unet_uri):
    return URI.from_uri_string(unet_uri)
