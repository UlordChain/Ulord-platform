import unetschema
from unetschema.base import b58encode, b58decode, validate_b58_checksum
from unetschema.hashing import double_sha256, hash160
from unetschema.error import InvalidAddress
from unetschema.schema import ADDRESS_LENGTH, ADDRESS_PREFIXES, PUBKEY_ADDRESS, SCRIPT_ADDRESS
...
def validate_address_prefix(addr_bytes):
    if ord(addr_bytes[0]) not in ADDRESS_PREFIXES[unetschema.BLOCKCHAIN_NAME].itervalues():
...
def hash_160_bytes_to_address(h160, addrtype=PUBKEY_ADDRESS):
    if addrtype == PUBKEY_ADDRESS:
        prefix = chr(ADDRESS_PREFIXES[unetschema.BLOCKCHAIN_NAME][PUBKEY_ADDRESS])
    elif addrtype == SCRIPT_ADDRESS:
        prefix = chr(ADDRESS_PREFIXES[unetschema.BLOCKCHAIN_NAME][SCRIPT_ADDRESS])
    else:
...
def address_to_hash_160(addr):
    ...
    if prefix == chr(ADDRESS_PREFIXES[unetschema.BLOCKCHAIN_NAME][PUBKEY_ADDRESS]):
